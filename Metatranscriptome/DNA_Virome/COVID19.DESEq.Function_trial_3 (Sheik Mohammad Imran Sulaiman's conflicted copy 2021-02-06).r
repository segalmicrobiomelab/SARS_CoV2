...
deseq <- function(metadata,counts,recounts,sampletype,comparison) {
        ...
        #Fix any NA logFC  
        resy <- resy %>% mutate(logFC = if_else(is.na(logFC), 0, logFC))                    
        #PLOT IT
            ...
            geom_point(size = ifelse(resy$adj.P.Val<0.2 & resy$logFC>0, 100000 * resy$abundance.2, ifelse(resy$adj.P.Val<0.2 & resy$logFC<0, 100000 * resy$abundance.1,5)),color="black",alpha=0.8,shape=21)+
            #geom_point(size = ifelse(resy$adj.P.Val<0.2 & resy$logFC>0, 200000 * resy$abundance.2, ifelse(resy$adj.P.Val<0.2 & resy$logFC<0, 200000 * resy$abundance.1,5)),color="black",alpha=0.8,shape=21)+
